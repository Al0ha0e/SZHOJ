        ...
        <v-col cols="6">
            <v-card>
                ...
                <v-card-text>
                    ...
                    <v-text-field :append-icon="showPassword2 ? 'mdi-eye' : 'mdi-eye-off'" :rules="[checkDuplicate]" :type="showPassword2 ? 'text' : 'password'" name="input-10-2" label="重复密码" class="input-group--focused" @click:append="showPassword2 = !showPassword2" v-model="password2"></v-text-field>
                    <v-btn color="green" class="ma-2 white--text" v-on:click="goLogin">返回登录</v-btn>
                    <v-btn color="blue" class="ma-2 white--text" v-on:click="register">注册</v-btn>
                </v-card-text>
            </v-card>
...
export default {
    ...
    methods: {
        ...
        register() {
            if (this.username.length > 15 || this.password != this.password2 || this.password.length > 15 || this.password.length < 8) {
            ...
            this.axios.post('http://127.0.0.1:8060/register', param).then((response) => {
