  ...
  <v-container>
    ...
    <v-dialog v-model="saveDialog" width="500">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn v-on:click="saveDialog = false" color="primary" text
            >确认</v-btn
          >
        </v-card-actions>
      </v-card>
  ...
  methods: {
    ...
    saveQuestion() {
      ...
      localStorage.setItem(
        "questionToCommit",
        JSON.stringify(questionToCommit)
      );
      this.saveDialog = true;
    },

    //从localStorage中恢复暂存的题目信息
